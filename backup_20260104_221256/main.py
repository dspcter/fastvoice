...
def create_tray_icon(app: FastVoiceApp, qt_app: QApplication) -> QSystemTrayIcon:
    ...
    # 恢复快捷键 - 用于长时间闲置后快捷键失效
    recover_action = QAction("恢复快捷键", qt_app)
    def recover_hotkeys():
        logger.info("用户触发快捷键恢复...")
        success = app.hotkey_manager.recover()
        if success:
            tray_icon.showMessage("快捷键已恢复", "请按快捷键测试功能", QSystemTrayIcon.MessageIcon.Information, 3000)
        else:
            tray_icon.showMessage("恢复失败", "请尝试重启应用", QSystemTrayIcon.MessageIcon.Warning, 5000)
    recover_action.triggered.connect(recover_hotkeys)
...
def main():
    ...
    # 添加应用心跳定时器（智能休眠：活跃期 60 秒，闲置期 300 秒）
    ...
    def heartbeat():
        ...
        # 检查是否处于闲置模式
        is_idle_mode = app.hotkey_manager._is_idle_mode

        # 单行日志输出 - 使用 lazy logging
        # 健康状态简化显示：正常=✓，可能问题=⚠
        health_symbol = '✓' if listener_status['health'] == '正常' else ('⚠' if listener_status['thread_alive'] else '✗')
        mode_str = "闲置" if is_idle_mode else "活跃"
        logger.info(
            "心跳 %ds | [%s模式] Watchdog:%s Listener:%s(%.0fs) 内存:%.1fMB",
        ...
        if need_recovery:
            logger.warning("检测到快捷键系统异常，尝试自动恢复...")
            success = app.hotkey_manager.recover()
    ...
    heartbeat_timer.start(60000)  # 60 秒（watchdog 会根据模式调整实际输出间隔）
