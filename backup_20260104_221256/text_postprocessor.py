...
class TextPostProcessor:
    ...
    def process(self, text: str) -> str:
        ...
        # 步骤3: 转换序数词（第2 → 第二）
        result = self._convert_ordinal_numbers(result)
    ...
    def _convert_ordinal_numbers(self, text: str) -> str:
        ...
        def number_to_chinese(num_str: str) -> str:
            ...
            try:
                ...
                if num == 0:
                    ...
                    return '十' + digit_map.get(num_str[1], '')
                elif num < 100:
                    tens = num // 10
                    ones = num % 10
        ...
        def replace_ordinal(match):
            ...
            chinese_num = number_to_chinese(number_str)
            return f"第{chinese_num}"

        # 使用正则表达式查找并替换
        # 模式：第 + 可选空格 + 一个或多个数字（处理"第2"和"第 2"两种情况）
        result = re.sub(r'第\s*(\d+)', replace_ordinal, text)

        if result != text:
            logger.info(f"序数词转换: '{text}' → '{result}'")
    ...
    def _convert_chinese_numbers(self, text: str) -> str:
        ...
        try:
            ...
            # ========== 新增：动词重叠模式 (V+一+V) ==========
            # 保护"动词 + 一 + 动词"的结构，如"听一听"、"看一看"
            # 常见动词叠词列表
            common_verbs = [
                '听听', '看看', '想想', '说说', '做做', '走走', '试试', '买买',
                '读读', '写写', '画画', '唱唱', '跳跳', '跑跑', '坐坐', '站站',
                '躺躺', '睡睡', '醒醒', '吃吃', '喝喝', '玩玩', '乐乐',
                '学学', '教教', '练练', '查查', '找找', '算算', '记记', '认认',
                '等等', '问问', '答答', '聊聊', '谈谈', '讲讲', '叫', '喊喊',
                '笑笑', '哭哭', '停停', '开开', '关关', '点点', '按按',
                '敲敲', '拍拍', '打打', '拿拿', '放放', '收收', '送送',
                '换换', '修修', '补补', '洗洗', '晒晒', '晾晾', '叠叠',
                '理理', '瞧瞧', '瞄瞄', '盯盯', '观望', '瞅瞅',
                '闻闻', '嗅嗅', '尝尝', '品品', '摸摸', '碰碰', '捏捏',
                '抓抓', '握握', '考虑', '搜搜', '测测', '验验', '考考',
            ]
            # 生成"动词+一+动词"模式（如：听一听）
            for verb in common_verbs:
                if len(verb) == 2:  # 处理"听听"这样的叠词
                    protected_patterns.append(f'{verb[0]}一{verb[0]}')
                    protected_patterns.append(f'{verb[0]}了{verb[0]}')  # 听了听
                    protected_patterns.append(verb)  # 听听、看看等
                elif len(verb) == 1:  # 单字动词
                    protected_patterns.append(f'{verb}一{verb}')
                    protected_patterns.append(verb)  # 单个动词

            # ========== 新增：数量概数表达 ==========
            # "三五个"、"七八个"、"十来个"、"两三个" 等
            for num1 in '一二三四五六七八九十两':
                for num2 in '一二三四五六七八九十两':
                    protected_patterns.append(f'{num1}{num2}个')  # 三五个
                    protected_patterns.append(f'{num1}{num2}天')  # 三五天
            # 添加"十来个"、"两三个"、"四五个"等变体
            for num in '一二三四五六七八九十两':
                protected_patterns.append(f'{num}来个')
                protected_patterns.append(f'{num}余个')
                protected_patterns.append(f'{num}多个')
                protected_patterns.append(f'数{num}个')
                protected_patterns.append(f'约{num}个')
                protected_patterns.append(f'几{num}个')  # 几个、几个
            ...
            protected_patterns.extend(idioms)

            # ========== 新增：时间日期表达 ==========
            # "星期一"、"星期二" 等
            for num in '一二三四五六七八九十日天':
                protected_patterns.append(f'星期{num}')
                protected_patterns.append(f'周{num}')
            # "五月一日"、"二零二四年" 等
            time_units = '年月日号'
            for num in '一二三四五六七八九十〇零两':
                for unit in time_units:
                    protected_patterns.append(f'{num}{unit}')  # 五月、一日
            # 保护"二零二四"这样的年份表达
            for num in '〇零一二三四五六七八九':
                protected_patterns.append(f'二零{num}')  # 二零二四
                protected_patterns.append(f'二零{num}{num}')  # 二零二三
            ...
            for adj in common_adj:
                protected_patterns.append(f'{adj}{adj}')  # 大大的
                protected_patterns.append(f'{adj}{adj}的')  # 大大的

            # ========== 原有保护规则 ==========

            # 保护"口语化数字 + 数字单位"的模式
            # 只保护"两"、"三"等小数字 + 单位（口语化表达）
            # 不保护"一万"、"一千"等正式数字（应该被转换）
            colloquial_nums = '两二三三四五六七八九'  # 口语化数字，不含"一"、"十"
            for num in colloquial_nums:
                for unit in '万千百亿兆':
            ...
            placeholders = []
            protected_text = text
            placeholder_counter = [0]  # 全局计数器，避免命名冲突
            ...
            for i, pattern in enumerate(protected_patterns):
                ...
                # 判断是否需要严格的边界保护
                # 对于包含中文数字的模式（如"听一听"、"三五个"），不需要边界限制
                # 因为这些模式本身就包含中文数字
                has_chinese_digit = any(c in pattern for c in chinese_digits_pattern)

                if has_chinese_digit:
                    # 对于包含中文数字的模式，直接匹配
                    # 不使用单词边界（因为 \b 对中文不起作用）
                    regex_pattern = pattern_escaped
                else:
                    # 对于纯词语（如"一些"、"一般"），使用严格的边界保护
                ...
                def make_replacer(p_pattern, counter):
                    def replacer(match):
                        # 使用计数器生成唯一的 placeholder
                ...
                protected_text = re.sub(regex_pattern, make_replacer(pattern, placeholder_counter), protected_text)

            # 保护序数词：第[中文数字]（如"第一"、"第二"、"第十"等）
            ordinal_pattern = r'第[零一二三四五六七八九十百千万亿两〇]+'
            ordinal_matches = list(re.finditer(ordinal_pattern, protected_text))
            for idx, match in enumerate(reversed(ordinal_matches), start=len(protected_patterns)):
