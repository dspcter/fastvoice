...
class HotkeyManager:
    ...
    # 闲置阈值：无键盘事件超过此时间（秒）进入闲置模式
    IDLE_THRESHOLD = 300  # 5 分钟

    # Listener 定期重建间隔（防止内存泄漏）
    LISTENER_REBUILD_INTERVAL_ACTIVE = 1800  # 活跃期：30 分钟
    LISTENER_REBUILD_INTERVAL_IDLE = 900     # 闲置期：15 分钟
    ...
    def __init__(self):
        ...
        # 智能休眠模式 - 根据活跃度调整检查频率
        self._is_idle_mode: bool = False  # 是否处于闲置模式
        self._last_listener_rebuild: float = time.time()  # 上次 listener 重建时间

        logger.info("快捷键管理器初始化完成 (P0 重构版: 状态机 + 防抖 + watchdog + 按键事件跟踪 + 智能休眠)")
    ...
    def _start_watchdog(self) -> None:
        ...
        def watchdog_loop():
            ...
            while self._watchdog_running:
                try:
                    ...
                    # ========== 智能休眠模式检测 ==========
                    # 每 30 秒检查一次是否需要切换模式
                    if current_time - last_mode_check > 30:
                        last_mode_check = current_time
                        time_since_last_key = current_time - self._last_key_event_time

                        was_idle = self._is_idle_mode
                        self._is_idle_mode = time_since_last_key > self.IDLE_THRESHOLD

                        if was_idle != self._is_idle_mode:
                            mode = "闲置模式" if self._is_idle_mode else "活跃模式"
                            logger.info(f"切换到 {mode} (无键盘事件 {time_since_last_key:.0f}秒)")

                    # 根据模式调整检查间隔
                    # 活跃模式：每 60 秒输出一次；闲置模式：每 300 秒 (5分钟) 输出一次
                    heartbeat_interval = 60 if not self._is_idle_mode else 300
                    if self._watchdog_loop_count % heartbeat_interval == 0:
                        ...
                        if self._listener:
                            try:
                                ...
                                listener_alive = False
                        mode_str = "闲置" if self._is_idle_mode else "活跃"
                        # 使用 lazy logging 避免字符串累积
                        logger.info("Watchdog %ds | [%s模式] Listener:%s",
                                   self._watchdog_loop_count,
                                   mode_str,
                                   '✓' if listener_alive else '✗')

                    # ========== 定期重建 Listener（防止内存泄漏）==========
                    # 根据模式使用不同的重建间隔
                    rebuild_interval = (
                        self.LISTENER_REBUILD_INTERVAL_IDLE if self._is_idle_mode
                        else self.LISTENER_REBUILD_INTERVAL_ACTIVE
                    )
                    time_since_rebuild = current_time - self._last_listener_rebuild

                    if time_since_rebuild >= rebuild_interval:
                        logger.info(f"定期重建 Listener（{time_since_rebuild:.0f}秒未重建）...")
                        import threading
                        rebuild_thread = threading.Thread(
                            target=self._restart_listener,
                            daemon=True,
                            name="ListenerRebuild"
                        )
                        rebuild_thread.start()
                        self._last_listener_rebuild = current_time

                    # ========== Listener 健康检查（活跃模式检查，闲置模式跳过）==========
                    health_check_interval = (
                        self.LISTENER_HEALTH_CHECK_INTERVAL if not self._is_idle_mode
                        else self.LISTENER_HEALTH_CHECK_INTERVAL * 3  # 闲置模式检查频率降低
                    )

                    if current_time - last_listener_check > health_check_interval:
                        ...
                        if self._listener:
                            try:
                                ...
                                # 静默失效阈值：30 分钟无任何按键事件才认为可能失效
                                # 原因：用户可能暂时不使用键盘，3 分钟太短会误报
                                is_silent_dead = time_since_last_key_event > 1800
        ...
        logger.info(
            "Watchdog 已启动 (超时: %ds, Listener检查: %ds, 心跳: 每60s/300s, "
            "Listener重建: 活跃%ds/闲置%ds, 闲置阈值: %ds)",
            self.WATCHDOG_TIMEOUT_S,
            self.LISTENER_HEALTH_CHECK_INTERVAL,
            self.LISTENER_REBUILD_INTERVAL_ACTIVE,
            self.LISTENER_REBUILD_INTERVAL_IDLE,
            self.IDLE_THRESHOLD
        )
    ...
    def _restart_listener(self) -> bool:
        ...
        当检测到 listener 线程死亡或静默失效时调用此方法尝试恢复

        改进:
        - 增加静默失效阈值到 30 分钟，避免误报
        - 接受 pynput 限制：线程存活即认为成功
        - 真正的验证需要用户实际测试快捷键
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # pynput 限制：无法完全验证 listener 是否接收事件
                # 接受线程存活作为成功标准
                # 真正的测试需要用户按下快捷键
                logger.info(f"✓ Listener 重启成功 (尝试 {attempt}/{max_retries})")
                logger.info("   提示：请按快捷键测试功能是否正常")
                return True
