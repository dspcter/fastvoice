...
class AudioCapture:
    ...
    def stop_recording(self) -> Optional[bytes]:
        ...
        try:
            ...
            # 停止音频流（改进版：使用非 daemon 线程确保资源释放）
            if self._stream:
                try:
                    # 使用非 daemon 线程确保资源释放完成
                    ...
                    def stop_stream():
                        try:
                            ...
                            logger.debug("音频流已正常停止并关闭")
                        except Exception as e:
                            stop_result['error'] = e
                            logger.warning(f"停止/关闭音频流时出错: {e}")
                        finally:
                            self._stream = None

                    # 启动停止线程（使用非 daemon，确保完成）
                    stop_thread = threading.Thread(target=stop_stream, daemon=False)
                    stop_thread.start()
                    ...
                    if not stop_result['done']:
                        logger.error("音频流停止超时（5秒），资源可能未完全释放")
                        logger.error("这可能导致下次录音失败，建议检查音频设备")
                        # 强制清理（但可能无法释放底层资源）
                        try:
                            if self._stream:
                                self._stream.close()
